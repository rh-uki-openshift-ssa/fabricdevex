...
image::000-image001.png[Example Topology Image]
...
Ensure that you are in the correct project for you. Select 'Project' from the top of the screen and check that you are using the project called %PROJECT%. If you are not then click on 'Create project' and create a project of that name. It is important to use that name as all further instructions will reference it.
...
In the *General* part of the form there are two textboxes; 'Application name' and 'Name'. This is slightly confusing at first; OpenShift has the concept, on the UI, of an Application name, which is a grouping mechanism for related applications. 

Enter *devex-app* as the name of the new Application name.
...
In the *Resources* part of the form you are given a drop down field to choose the resource type to generate. In this case it is a simple choice between a *Deployment* object or a *DeploymentConfig* object.
...
For this example choose *Deployment*. 
